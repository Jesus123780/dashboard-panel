...
export const TextAreaHooks = ({
    ...
    useEffect(() => {
        if ((value?.length > range?.max)) {
            setAlertBox({ message: `El rango no debe superar los ${ range.max } caracteres`, duration: 5000 })
        }
    }, [value])
    return (
        <BoxInput width={width} padding={padding} minWidth={minWidth}>
            ...
            { showRange && <span>{ value?.length ? value?.length : 0 } / {range?.max}</span>}
            <br></br>
            { showRange && <span>{ value?.length ? value?.length - range?.max : range?.max }</span>}
            {/* <IconWarning size={20} color={PColor} style={{ position: 'absolute', right: 5, bottom: 10, opacity: 0 }} /> */}
        </BoxInput>
    )
}

TextAreaHooks.propTypes = {
